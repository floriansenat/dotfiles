...
- When adding TODO comments always add my Gitlab alias: `florian.senat` in parenthesis
    - Example: `TODO(florian.senat):`
...
- Before creating a plan, launch a `Sentinel` sub-agent to define the testing strategy
...
- If file changes are in `app` folder, you MUST `cd` into `app` first before launching `rtk pnpm` scripts
- To target a specific package, use the `--filter` flag. Exemple: `rtk pnpm --filter "@yousign/main" <command>`
- To test use `rtk pnpm --filter "<package>" test`
- To check types errors use `rtk pnpm --filter "<package>" check:types`
- To lint with biome use `rtk pnpm --filter "<package>" check:error`
- To fix with biome use `rtk pnpm --filter "<package>" fix:error`
